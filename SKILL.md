...
- Normalize route phrasing before solving: preserve explicit airport codes, expand city/airport names through `map.search_locations`, and convert natural-language connectors (`to`, `from`, `via`, arrows, dashes, commas) into an ordered waypoint list.
...
- `map.render_svg`: use for final export-ready output, not exploration. Pair it with share-packet guidance when the result will be reused outside the chat/session.
...
1. Identify route-like connectors (`-`, `→`, `to`, `from`, `via`, `through`, commas, or whitespace between known airport tokens).
...
4. Preserve user-stated waypoint order, including `via` stops, and do not render until every waypoint resolves to one intended airport or coordinate.
5. For ambiguous or malformed input, read [references/input-understanding.md](references/input-understanding.md) and give one concise clarification or correction hint, for example: `I need at least two airports or cities, like JFK-LHR or New York to London.`

## Resources

- Read `circulus://mapspec/schema` before authoring a non-trivial `MapSpecV1`.
- Read [references/input-understanding.md](references/input-understanding.md) when route phrasing is ambiguous, malformed, city-name-heavy, or mixes prose with airport codes.
...
- Read [references/export-shareability.md](references/export-shareability.md) when the user wants SVG export, briefing-ready output, documentation embedding, or any reusable handoff of the rendered map.
...
- For route input normalization, ambiguity handling, and malformed-input feedback, read [references/input-understanding.md](references/input-understanding.md).
...
- For export/share packets and reusable SVG handoff wording, read [references/export-shareability.md](references/export-shareability.md).
